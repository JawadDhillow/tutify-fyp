...
<form method="post" action="process_payment.php">
  <div class="form-group">
    ...
    <select class="form-control" id="tutor_id" name="tutor_id" required>
      ...
      <?php
        include('../dbcon.php');
        $sql = "SELECT instructor_id FROM payment WHERE salary_check='0'";
        $result = mysqli_query($con, $sql);
        while ($row = mysqli_fetch_assoc($result)) {
  ...
  <div class="form-group">
    ...
    <select class="form-control" id="tutor_name" name="tutor_name" required onchange="updateTutorId()">
      <option value="">Select a tutor name</option>
      <?php
        include('../dbcon.php');
        $sql = "SELECT instructor_id, course_instructor FROM payment WHERE salary_check='0'";
        $result = mysqli_query($con, $sql);
        while ($row = mysqli_fetch_assoc($result)) {
          echo "<option value='".$row['course_instructor']."'>".$row['course_instructor']."</option>";
        }
        mysqli_close($con);
      ?>
    </select>
  ...
  <script>
  function updateTutorId() {
    var tutorName = document.getElementById("tutor_name").value;
    var tutorIdDropdown = document.getElementById("tutor_id");

    // Loop through the options in the tutor ID dropdown
    for (var i = 0; i < tutorIdDropdown.options.length; i++) {
      var option = tutorIdDropdown.options[i];

      // Check if the tutor name matches the option text
      if (option.text === tutorName) {
        option.selected = true; // Select the matching tutor ID
        break;
      }
    }
  }
